...
img.onload = function(){
  loaded = true;
	...
	imgWidth = this.width || this.naturalWidth;
	imgHeight = this.height || this.naturalHeight;

	imgCanvas = new Canvas(imgWidth, imgHeight);
  ...
  imgCtx.drawImage(img, 0, 0, this.width, this.height);
};

let raytracerTimer = setInterval( function() {
  if (loaded){
    loaded = false;
    raytracer();
  }
...
function pixel(x, y) {
  ...
  if (debug) console.log("with ray g= (" + g + "), d=(" + d + ")");

  return ray_color(g, d);
}

function raytracer(){
  clearInterval(raytracerTimer);
